...
test("diagnostic report is stable, complete, and sanitizes sensitive detail", () => {
  const report = formatter()({
    generatedAt: "2026-09-09T00:00:00.000Z", reportId: "report-1", overall: "degraded", capabilities: [{ id: "notion-sync", label: "Notion Sync", status: "unavailable", enabled: true, sourceCheckIds: ["notion-authentication"], reason: "Notion authentication: token=do-not-copy" }],
    recentFailures: [], lastSyncResults: [],
  ...
  assert.match(report, /Overall health: DEGRADED\n\nCapabilities\nNotion Sync: UNAVAILABLE — Notion authentication: token: \[redacted\]/);
