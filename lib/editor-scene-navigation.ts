type Volume = { id: string; novelId: string; sortOrder: number; archived?: boolean };
type Chapter = { id: string; volumeId: string; sortOrder: number; archived?: boolean };
...
export function getNovelSceneNavigation(novelId: string, volumes: Volume[], chapters: Chapter[], scenes: Scene[]) {
  const volumesByOrder = volumes.filter((v) => v.novelId === novelId && !v.archived).sort((a, b) => a.sortOrder - b.sortOrder || a.id.localeCompare(b.id));
  const volumeOrder = new Map(volumesByOrder.map((v, i) => [v.id, i]));
  const chaptersByOrder = chapters.filter((c) => !c.archived && volumeOrder.has(c.volumeId)).sort((a, b) => (volumeOrder.get(a.volumeId)! - volumeOrder.get(b.volumeId)!) || a.sortOrder - b.sortOrder || a.id.localeCompare(b.id));
